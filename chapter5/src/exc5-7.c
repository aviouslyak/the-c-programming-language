...
int readlines(char *lineptr[], char *buff, char **pos, int nlines);
...
int main() {
    ...
    char buff[ALLOCSIZE];
    char *pos = buff;
    if ((nlines = readlines(lineptr, buff, &pos, MAXLINES)) >= 0) {
...
int readlines(char *lineptr[], char *buff, char **pos, int maxlines) {
    int len, nlines;
    char *p, line[MAXLEN];
